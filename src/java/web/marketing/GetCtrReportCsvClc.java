...
@MultipartConfig
@WebServlet(name = "ctrReportCsvClc", urlPatterns = {"/ctrReportCsvClc"})
public class GetCtrReportCsvClc extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection con = MySQLCommon.getConnection("sigmads", null);) {
            ...
            if (cancel && anul) {
                throw new Exception("No pueden ser Anulados y Cancelados a la vez.");
            }
            ...
            str.append("contract.contract_num, ")//0
                    ...
                    .append((aditional ? " CONCAT(first_name, ' ', COALESCE (last_name,''))," : ""))// a1
                    .append((aditional ? "address," : ""))//  a2
                    .append((aditional ? "phones," : ""))// a3
                    .append((smbEmail && aditional ? "email," : ""))// a4 
                    .append("contract.deposit, ")//7 + a4 
                    .append("(SELECT CONCAT(first_name,' ',last_name) FROM inv_store store WHERE store.id = contract.inv_store_id), ") //8 + a4
                    .append("(SELECT CONCAT(first_name,' ',last_name) FROM employee emp WHERE emp.id = contract.sower), ") //9 +a4
                    .append("(SELECT CONCAT(first_name,' ',last_name) FROM employee emp WHERE emp.id = contract.writer), ") //10 +a4
                    .append("(SELECT veh.plate FROM vehicle veh WHERE veh.id = contract.vehicle_id), "); //11 +a4
            ...
            if (cancel && !noCancel) {
                str.append("cancel_date BETWEEN '").append(begin).append("' AND '").append(end).append("' AND ");
            } else if (cancel && noCancel) {
                str.append("(cancel_date BETWEEN '").append(begin).append("' AND '").append(end).append("' OR ")
                        .append("sign_date BETWEEN '").append(begin).append("' AND '").append(end).append("') AND ");
            ...
            out.write("Contrato;Cédula;Zona;Poblado;DaneCod;Fecha;Adjuntos;" + (aditional ? "Barrio;Nombres;Dirección;Teléfono;" + (smbEmail && aditional ? "Correo;" : "") : "") + "Deposito;Almacen;Sembrador;Digitador;Vehículo;" + cylNames.toString() + "Total" + System.lineSeparator());
            ...
            while (rs.next()) {
                ...
                for (int j = 0; j < cols; j++) {
                    ...
                    } else if (j == 7 + (aditional ? (smbEmail ? 5 : 4) : 0)) { // deposito 
                    ...
                    strRow += ";";
                    if (j >= (12 + (aditional ? (smbEmail ? 5 : 4) : 0))) {// empieza a sumar las cantidades 
            ...
            try {
                ...
                response.setStatus(500);
                Logger.getLogger(GetCtrReportCsvClc.class.getName()).log(Level.SEVERE, null, ex1);
            }
        } finally {
            try {
                out.close();
