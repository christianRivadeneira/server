...
@MultipartConfig
@WebServlet(name = "getSalesDetail", urlPatterns = {"/getSalesDetail"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (Connection con = MySQLCommon.getConnection("sigmads", null)) {
            ...
            String str = "SELECT "
                    ...
                    + "IF(ts.cyl_received_id IS NOT NULL, CONCAT(LPAD(rc.nif_y, 2, 0), LPAD(rc.nif_f, rc.fac_len, 0), LPAD(rc.nif_s, 6, 0)), IF(!ts.is_sowing, (SELECT GROUP_CONCAT(CONCAT(LPAD(tcr.nif_y, 2, 0), LPAD(tcr.nif_f, tcr.fac_len, 0), LPAD(tcr.nif_s, 6, 0))) FROM trk_multi_cyls mrc LEFT JOIN trk_cyl tcr ON mrc.cyl_id = tcr.id WHERE mrc.sale_id = ts.id AND mrc.`type` = 'rec'), NULL)), "
                    + "ts.lat, "
                    + "ts.lon "
                    + "FROM trk_sale ts "
            ...
            try {
                ...
                response.setStatus(500);
                Logger.getLogger(GetSalesDetail.class.getName()).log(Level.SEVERE, null, ex1);
            }
        } finally {
            try {
                if (out != null) {
