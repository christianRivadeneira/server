...
@MultipartConfig
@WebServlet(name = "EvidenceFileManager", urlPatterns = {"/EvidenceFileManager"})
public class EvidenceFileManager extends HttpServlet {
...
//                    int bFileId= Integer.parseInt(convertStreamToString(request.getPart("bFileId" + i).getInputStream()));
                ...
                if (sigmaId > 0) {
                    String query = "UPDATE bfile SET "
                            ...
                            + ",keywords = ''"
                            + "WHERE id = " + sigmaId;
                    new MySQLQuery(query).executeUpdate(con);
                    idsJar.add(sigmaId);
                } else {
                    String query = "INSERT INTO bfile SET "
                            ...
                            + ",updated = NOW() "
                            + ",keywords = ''";
                    int id = new MySQLQuery(query).executeInsert(con);
            ...
            Logger.getLogger(EvidenceFileManager.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(EvidenceFileManager.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
