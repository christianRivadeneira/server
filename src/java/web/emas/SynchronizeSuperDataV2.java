...
@MultipartConfig
@WebServlet(name = "SynchronizeSuperDataV2", urlPatterns = {"/SynchronizeSuperDataV2"})
public class SynchronizeSuperDataV2 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (JsonWriter w = Json.createWriter(response.getOutputStream())) {
            ...
            try {
                ...
                JsonArray schedules = object.getJsonArray("schedules");
                JsonArray progs = object.getJsonArray("progs");
                ...
                for (int i = 0; i < schedules.size(); i++) {
                    ...
                    int vhId = schedule.getInt("vhId", 0);
                    if (vhId == 0) {
                        MySQLQuery q = new MySQLQuery("DELETE FROM emas_schedule WHERE id = " + schedule.getInt("id"));
                        q.executeDelete(conn);
                    } else {
                        MySQLQuery q = new MySQLQuery("UPDATE emas_schedule SET vh_id = " + vhId + " WHERE id = " + schedule.getInt("id"));
                        q.executeUpdate(conn);
                    }
                }
                for (int i = 0; i < progs.size(); i++) {
                    JsonObject prog = progs.getJsonObject(i);
                    int vhId = prog.getInt("vhId", 0);
                    if (vhId == 0) {
                        sendMail = true;
                        String obj = new MySQLQuery("SELECT name FROM emas_prog p INNER JOIN emas_path pa ON p.path_id = pa.id WHERE p.id = " + prog.getInt("id")).getAsString(conn);
                        path += "\n- Se quedó sin asignación de vehículo la ruta: " + obj;
                        MySQLQuery q = new MySQLQuery("DELETE FROM emas_prog WHERE id = " + prog.getInt("id"));
                        q.executeDelete(conn);
                    } else {
                        MySQLQuery q = new MySQLQuery("UPDATE emas_prog SET vh_id = " + vhId + " WHERE id = " + prog.getInt("id"));
                        q.executeUpdate(conn);
                    }
                }
            } catch (Exception ex) {
                Logger.getLogger(SynchronizeSuperDataV2.class.getName()).log(Level.SEVERE, null, ex);
                response.sendError(500);
                ob.add("status", STATUS_ERROR);
                ob.add("msg", ex.getMessage());
            } finally {
                if (sendMail && !path.isEmpty()) {
                    String emps = new MySQLQuery("SELECT emp_mail FROM emas_cfg WHERE id = 1").getAsString(conn);
                    String mails = new MySQLQuery("SELECT GROUP_CONCAT(mail) FROM employee WHERE id IN (" + emps + ")").getAsString(conn);
                    msg = "Se Realizó el cambio de rutas con las siguientes novedades: \n" + path;
                    SendMail.sendMail(conn, mails, "Cambió de Rutas", msg, msg);
                }
