...
@WebServlet(urlPatterns = {"/GetJvmFile"})
public class GetJvmFile extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        Map<String, String> pars = MySQLQuery.scapedParams(request);

        try (Connection con = MySQLCommon.getConnection(pars.get("poolName"), null); GZIPOutputStream gos = new GZIPOutputStream(response.getOutputStream())) {
            String path = new MySQLQuery("SELECT jvm_path FROM sys_cfg").getAsString(con);
            File file = new File(path + File.separator + pars.get("file"));
