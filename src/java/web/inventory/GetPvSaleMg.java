...
@MultipartConfig
@WebServlet(name = "getPvSaleMg", urlPatterns = {"/getPvSaleMg"})
public class GetPvSaleMg extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (JsonWriter w = Json.createWriter(response.getOutputStream())) {
            try (Connection con = MySQLCommon.getConnection("sigmads", null)) {
                ...
                Logger.getLogger(GetPvSaleMg.class.getName()).log(Level.SEVERE, null, ex);
            } finally {
                w.writeObject(ob.build());
                System.out.println("GetPvSaleMg " + (System.currentTimeMillis() - t) + "ms");
            }
        } catch (Exception ex) {
            Logger.getLogger(GetPvSaleMg.class.getName()).log(Level.SEVERE, null, ex);
        }
