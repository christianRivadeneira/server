...
public class DailyGoalProcessor extends LogicProcessor {

    public DailyGoalProcessor() {
    }

    @Override
    public List<Query> createQueriesAccumulated(EntityManager em) {
        ArrayList<Query> res = new ArrayList<Query>();
        res.add(em.createNativeQuery("SELECT (sum(v1)/ sum(v2))*100 FROM ind_value WHERE ?1 <= when_value AND ?2 >= when_value AND scale_id = ?3 AND agency_id = ?4"));
        return res;
    }
