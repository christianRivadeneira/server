...
public class NoteWriter {
    ...
    public NoteWriter(BillInstance inst, Connection conn) throws Exception {
        this.inst = inst;
    ...
    public void addNote(BillNote note, BillClientTank client, BillBuilding build, BillSpan span, BillBank bank, City city, List<BillTransaction> lst) throws Exception {
        if (currBill > 1) {
            document.newPage();
            preparePage();
            currBill = 0;
    ...
    public void drawNote(BillNote note, BillClientTank client, BillBuilding build, BillSpan span, BillBank bank, City city, List<BillTransaction> lst, int j) throws Exception {
        ...
        for (int i = 0; i < lst.size(); i++) {
            BillTransaction trans = lst.get(i);
            String acc;
            switch (note.typeNotes) {
                case "n_cred":
                case "aj_cred":
            ...
            cb.setFontAndSize(bf, 9);
            addTextRight("car" + (i + 1), moneyFormat.format(trans.value), j);
            total = total.add(trans.value);
        }
