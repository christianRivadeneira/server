...
public class TankPQRWriter {
    ...
    public TankPQRWriter(Connection conn) throws Exception {
        this.conn = conn;
    }

    public void beginDocument(OrdPqrTank bill, OrdCfg cfg) throws Exception {
        //create fonts
        this.cfg = cfg;
    ...
    private Map<String, float[]> getPositions() throws Exception {
        ...
        PdfReader reader = new PdfReader(Reports.getPatternStream("tankPQR.pdf", this));
    ...
    private void drawFact(OrdPqrTank bill) throws Exception {
        ...
        String pqrFec = df.format(bill.registDate);
        ...
        if (bill.buildId != null) {
            OrdTankClient build = new OrdTankClient().select(bill.buildId, conn);
            clientName = build.name;
            addr = build.address;
            phones = build.phones;
        } else {
            OrdPqrClientTank client = new OrdPqrClientTank().select(bill.clientId, conn);
            if (client.buildOrdId != null) { // Usuario Fact 
                OrdTankClient build = new OrdTankClient().select(client.buildOrdId, conn);
                addr = build.address + " " + build.name;

            } else { // Usuario Redes 
                Neigh neigh = new Neigh().select(client.neighId, conn);
        ...
        OrdPqrReason reason = new OrdPqrReason().select(bill.reasonId, conn);

        OrdTechnician tec = new OrdTechnician().select(bill.technicianId, conn);
        String tecName = tec.firstName + " " + tec.lastName;
        //Neigh neigh = ep.getAgencyService().getNeigh(index.getNeighId());
    ...
    private class PdfPageEventBill implements PdfPageEvent {
        ...
        public void onOpenDocument(PdfWriter writer, Document dcmnt) {
            try {
                ...
                Logger.getLogger(TankPQRWriter.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
